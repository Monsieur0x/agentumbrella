...
### Режимы работы бота
- **Активный** — отвечает на все сообщения, выполняет функции координатора
- **Наблюдение** — отвечает только на прямые @упоминания фиксированной фразой
- **Чат** — свободная болтовня на любые темы без функций координатора (отдельная модель через `CHAT_MODEL`)
...
CHAT_MODEL=claude-haiku-4-5-20251001  # модель для чат-режима (по умолчанию = MODEL)
...
**Управление (владелец):**
- «Режим наблюдение» / «Рабочий режим» / «Режим чат»
...
│   ├── brain.py               # Claude API, function calling, история диалогов, чат-режим
│   ├── system_prompt.py       # Динамический системный промпт по роли + чат-промпт
